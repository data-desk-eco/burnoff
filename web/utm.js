...
(function() {
    ...
    var M1 = 1 - e2/4 - 3*e2*e2/64 - 5*e2*e2*e2/256;
    var M2 = 3*e2/8 + 3*e2*e2/32 + 45*e2*e2*e2/1024;
    var M3 = 15*e2*e2/256 + 45*e2*e2*e2/1024;
    var M4 = 35*e2*e2*e2/3072;
    ...
    self.wgs84ToUtm = function(lon, lat, zone, isNorth) {
        var rad = Math.PI / 180;
        var phi = lat * rad;
        var lambda0 = centralMeridianRad(zone);
        var dlambda = lon * rad - lambda0;
        ...
        var A = cosPhi * dlambda;
        var M = meridionalArc(phi);

        var A2 = A*A, A3 = A2*A, A4 = A2*A2, A5 = A4*A, A6 = A4*A2;
    ...
    self.utmToWgs84 = function(easting, northing, zone, isNorth) {
        ...
        var es = e2 * sinPhi1 * sinPhi1;
        var N1 = a / Math.sqrt(1 - es);
        var T1 = tanPhi1 * tanPhi1;
        var C1 = ep2 * cosPhi1 * cosPhi1;
        var R1 = a * (1 - e2) / Math.pow(1 - es, 1.5);
        var D = x / (N1 * k0);
